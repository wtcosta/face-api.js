...
class FaceHashGenerator {
   ...
   */
  static generateSimpleHash(descriptor) {
   ...
   */
  static generateMD5Hash(descriptor) {
    const descriptorString = Array.from(descriptor).join(',')
   ...
   */
  static generateBase64Hash(descriptor) {
    const descriptorString = Array.from(descriptor).join(',')
   ...
   */
  static generateHexHash(descriptor) {
   ...
   */
  static generateFaceHash(descriptor, options = { type: 'simple' }) {
    let hash
   ...
   */
  static generateAllHashes(descriptor) {
    return {
...
async function run() {
  ...
  const hashTypes = ['simple', 'sha256', 'md5', 'base64', 'hex']
